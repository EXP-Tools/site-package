			...
			if(temp=="    .参数"||temp=="    .成员"){//Dll参数，数据类型成员
				...
				if(temp.substr(0,1)=="."){//处理系统流程命令
					...
					if(temp.indexOf("版本")==-1&&temp.indexOf("支持库")==-1){
						...
						if(wm==1){linetext[a]=_blank.replace(/[ ]/g,"&nbsp;")+"&nbsp;&nbsp;&nbsp;&nbsp;"+temp_r.fontcolor(color_sys_command_ignore)+temp_;}
						else{linetext[a]=_blank.replace(/[ ]/g,"&nbsp;")+"&nbsp;&nbsp;&nbsp;&nbsp;"+temp_r.fontcolor(color_sys_command)+temp_;}
						temp=linetext[a];
					}
					else{temp=""}
				}
